	\section{Entwurf und Begründungen}
		...
		\subsection{Konzeptionelle Domäne}
			...
			\begin{figure}[H]
				...
				]{ADRepo-Domäne} %TODO: gell Tobi, das ist vom ADRepo?
				\label{fig:dksDomain}
			\end{figure}
		...
		\subsection{Metamapping}
			...
			Beim Erzeugen von konkreten Tasks müssen zu jedem Mapping alle Occurrences (siehe ADRepo-Domäne \ref{fig:dksDomain}) gefunden werden. %TODO: gell, das war ADRepo und nicht generell DKS?
		...
		\decision{
			\decisionHeader{DOM-TT-STRUC}{\tt s Strukturierung}{Architecture}{Domain}
		}{
			...
			{Welches Konzept soll zur Strukturierung von \tt s eingesetzt werden?}
			{keine Speziellen}{Benutzer sollen einfach und schnell erstellte \tt s wieder finden.}
			{
				\begin{description}
					\item[Keine Strukturierung] \
						\begin{description}
							\item[Vorteile] Einfach zu implementieren, einfach verständlich für den Benutzer
							\item[Nachteile] Bei vielen \tt s unübersichtlich, führt zu doppelten \tt s,
					...
					\item[Direkte Hierarchisierung der \tt s] \
					...
					\item[Prototyping statt Strukturierung] \
						...
						\begin{description}
							\item[Vorteile] Verringert die Anzahl \tt s massiv, 
			...
			}
			{
				...
				und der Benutzer findet trotzdem dank Suchfunktionen die gesuchten \tt s.
				Ausserdem muss er keine Zeit aufwenden um die \tt s zu strukturieren.
		...
		\subsection{Verknüpfungen von \tt s und Entscheidungs-Vorlagen}
			...
			\subsection{Übertragung von \tt s in \ppt}
				...
				Beim Übertragen werden aus \tt s (konkrete) Tasks.
