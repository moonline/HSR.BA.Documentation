\chapter{Ergebnisse}
	...
	\subsection{Rückblick auf die Fragestellungen der Aufgabenstellung}
		\subsubsection{Wie lässt sich Entscheidungsbedarf in Form von agilen Planungsitems darstellen?}
			...
			Zusammengefasst stellt der Benutzer Entscheidungsbedarf in Form von agilen Planungsitems dar, indem er diese in \eeppi\ modelliert und übertragen lässt in ein Projektplnaungstool, wo sie verwaltet und weiterverarbeitet werden können.
	...
	\subsection{Erfolgsfaktoren}
		...
		\subsubsection{Niedrige Einstiegshürden für User}
			...
			Erster Aspekt ist ein geringer Installationsaufwand.
		...
		\subsubsection{Reife der Konzepte}
			...
			das Übertragen von Tasks an ein \ppt\ ist sogar durch den Benutzer im Frontend auf hohem Level anpassbar.
	...
	\section{Erweiterungs-Möglichkeiten}
		\eeppi\ zeigt Möglichkeiten und Wege, wie Entscheidungsmanagement und Projektplanung zusammengebracht werden kann. Diesen neuen Möglichkeiten wecken wiederum Begehrlichkeiten und Ideen.
		...
		\subsection{Erweiterung der bestehenden Applikation}
			...
			Beschränken sich die Anpassungen nur auf de Funktionalität und nicht auf die menge der persistieren Daten, so muss lediglich die Clientapplikation angepasst werden.
	
		\subsection{Ersatz der Serverapplikation}
			Die Serverapplikation verwaltet primär die Persistenz und stellt diese Funktionalität in Form der API zuer Verfügung.
		...
		\subsection{Ersatz der Clientapplikation}
			...
			Die neue Applikation muss lediglich die Serverschnittstelle implementieren.
			Auch ein Erweitern der Funktionalität durch eine parallele Clientapplikation ist denkbar.
	...
	\section{Mögliche Erweiterungen}
		...
		\subsection{Vererbende Tasktemplate}
			...
			Dem Elterntemplate untergeordnete Subtasks wie z.B. "'Zur Sitzung einladen"' oder "'Sitzungs-Protokoll"' versenden, würden automatisch auch auf Subtemplates angewandt, bzw. mit diesen übertragen ans \ppt.
			
		
		\subsection{Reporting}
			Eine weitere Erweiterungsmöglichkeit für \eeppi\ stellt eine Übersicht über in das \ppt\ exportierten Tasks dar.
		...
		\subsection{Verwendung mehrerer DKS}
			...
			Eine mögliche Erweiterung wäre die integration mehrere konfigurierbarer \dks e als Datenquelle.
		...
		\subsection{Verwendung mehrerer Projekte}
			...
			da zusammen mit dem Rechte-Rollen Konzept einzelne Benutzern nur bestimmten Projekte zugeteilt werden könnten.
		...
		\label{subsec:informationFlowbackFeature}
			...
			Unter Umständen gäbe es gar die Möglichkeit, dies teilautomatisiert zu tun oder zumindest zu erkennen und dem Benutzer vorzuschlagen.
		...
		\label{subsec:informationFlowbackFeatureDKS}
			...
			Wird beispielsweise im \ppt\ der Task "'Session State Evaluieren"' geschlossen und "'DB Session"' als Option usgewählt, 
			...
			Dem Benutzer würde diese Automatisierung Mehrspuringkeiten in verschiedenen Tools ersparen.
